...
Question 1.
Line 8 saying `mytickers = c("SPY", "AAPL")` in `dataSet1.R`

Question 2.
The cumulative returns is `r round(100*sum(StockReturns[,2]),1)`%.
The average daily returns is `r round(100*mean(StockReturns[,2]),3)`%.
The standard deviation of the daily returns is `r round(100*sd(StockReturns[,2]),3)`%;

Question 3.
Change the line8 in `dataSet1.R` to `mytickers = c("SPY", "AAPL", "YHOO")` and extract the data for Yahoo! as it was done for Apple in Q2.
...
First let's create the new horixontal matrix, transposedData.

```{r eval=TRUE, echo=TRUE, error = FALSE, warning=FALSE,message=FALSE,results='asis'}
transposedData = t(StockReturns)
```

Then we can use `nrow(transposedData)` to get the number of rows, which is `r nrow(transposedData)`, and we can use `ncol(transposedData)` to get the number of columns, which is `r ncol(transposedData)`.

Question 2.

We need to take mean of columns of the transposed matrix.

```{r eval=TRUE, echo=TRUE, error = FALSE, warning=FALSE,message=FALSE,results='asis'}
transposed_portfolio = apply(transposedData,2,mean)
names(transposed_portfolio) <- rownames(StockReturns)
```

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
pnl_plot(transposed_portfolio)
```
...
Question 1.
We need to change the line 9 `startDate = "2001-01-01"` in `dataSet1.R`

Question 2.
Yes, by changing the line 8 `mytickers = c("SPY", "AAPL")` in `dataSet1.R`
